...
/* =========  下面整段貼在 463 行以下（不要改上方程式） ========= */

const quizLoaded = { A:false, B:false, C:false };

// 取得 slug（和上方播放器取得方式一致）
...
function renderQuizSet(kind, listOrErr) {
  ...
  listEl.innerHTML = '';
  if (listOrErr?.error) {
    statusEl.textContent = `⚠️ 無法載入：${listOrErr.error}`;
    return;
  }

  const qs = listOrErr || [];
  if (!qs.length) {
    statusEl.textContent = '⚠️ 查無題目。';
    return;
  }
  ...
  qs.forEach(q => {
    ...
    if (q.type === 'MCQ') {
      ...
      body += `
        ...
        <button class="btn" data-act="check" style="margin-top:6px">檢查</button>
        <div class="exp" style="color:#9fb3d9;margin-top:6px">正解：<span class="ans"></span></div>
      `;
    } else { // SA
      body += `
        ...
        <button class="btn" data-act="check" style="margin-left:8px">檢查</button>
        <div class="msg" style="margin-top:6px"></div>
        <div class="exp" style="color:#9fb3d9;margin-top:6px">正解：<span class="ans"></span></div>
      `;
    }
...
async function ensureQuizLoaded(kind) {
  ...
  const statusEl = document.getElementById(`quiz${kind}Status`);
  if (statusEl) statusEl.textContent = `(測驗 ${kind} 載入中…)`;
  const data = await loadQuizSet(kind);
  renderQuizSet(kind, data);
  quizLoaded[kind] = true;
}

// 監聽你原本的分頁點擊機制：只要切到 quizA / quizB / quizC 就載入一次
